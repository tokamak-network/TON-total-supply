 ...
 * It retrieves Transfer logs from public RPC endpoints (see utils/publicRpc.js) to calculate
 * the locked and spent TON amounts within a specified block range.
...
const lockedTON = async (Block1, Block2) => {
  ...
  const TON_INTERFACE = new ethers.Interface(TONABI);
  ...
  const lockedLogs = await getLogsViaPublicRpc(
    startBlock,
    endBlock,
    TONContractAddress,
    TON_LOCKED_TOPICS
  ...
  const spentLogs = await getLogsViaPublicRpc(
    startBlock,
    endBlock,
    TONContractAddress,
    TON_SPENT_TOPICS
