 ...
 *
 * The `burnedSeignorage` function retrieves UnstakeLog events from public RPC
 * endpoints (see utils/publicRpc.js) and calculates the burned seignorage by
 * parsing the data from the logs.
 *
 * This module can be used to track and analyze the burned seignorage over time in the TON blockchain.
 */
const { ethers } = require("ethers");
const { getLogsViaPublicRpc } = require("./utils/publicRpc");
...
const burnedSeignorage = async (Block1, Block2) => {
  ...
  const SEIGMANGER_INTERFACE = new ethers.Interface(seigManagerABI);
  ...
  if (startBlock < patchedStartBlock) {
    if (endBlock < patchedStartBlock) {
      unstakeLogs = await getLogsViaPublicRpc(
        startBlock,
        endBlock,
      ...
      patchedUnstakeLogs = await getLogsViaPublicRpc(
        patchedStartBlock,
        endBlock,
